...
void *kmem_alloc_blocks(size_t num_blocks) {
    ...
    if (remaining_blocks > 1) {
        ...
        if (left_block->r_prev_free) left_block->r_prev_free->r_next_free = new_block;
        else free_list_head = new_block;
        if (left_block->r_next_free) left_block->r_next_free->r_prev_free = new_block;
        ...
        if (left_block->r_prev_free) left_block->r_prev_free->r_next_free = left_block->r_next_free;
        else free_list_head = left_block->r_next_free;
        if (left_block->r_next_free) left_block->r_next_free->r_prev_free = left_block->r_prev_free;
...
int kmem_free(void *ptr) {
    ...
    if (left_block->r_is_free) return -EINVAL;
    MetaBlock *right_block = at_offs(left_block, left_block->r_num_blocks + 1);
    ...
    if (left_free && right_free) {
        ...
        else free_list_head = prev_block;
    } else if (right_free) {
        MetaBlock *next_block = at_offs(right_block, right_block->r_num_blocks + 1);
        ...
        if (right_block->r_next_free) right_block->r_next_free->r_prev_free = left_block;
        if (right_block->r_prev_free) right_block->r_prev_free->r_next_free = left_block;
        else free_list_head = left_block;
    } else if (left_free) {
        MetaBlock *prev_block = at_offs(left_block, -left_block->l_num_blocks - 1);
        ...
        if (free_list_head) free_list_head->r_prev_free = left_block;
